...
export const removeUserByAdmin = async (req: Request, res: Response) => {
  try {
    ...
    console.log("user_id to remove:", user_id);
    const query = `DELETE FROM users WHERE user_id=$1 RETURNING *`;
    const result = await pool.query(query, [user_id]);
    console.log("result of delete query:", result);
    if (result.rows.length > 0) {
      return messageResponse({ res, status: 200, message: "user removed successfully", data: [], error: false });
    }
    return messageResponse({ res, status: 404, message: "user not found", data: [], error: true });
  } catch (error) {
    return messageResponse({ res, status: 500, message: "internal server error", data: [], error: error });
  }
}